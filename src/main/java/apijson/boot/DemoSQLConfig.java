...
public class DemoSQLConfig extends APIJSONSQLConfig<Long> {
	...
	static {
		DEFAULT_DATABASE = DATABASE_MYSQL;  // 使用真实MySQL数据库
		DEFAULT_SCHEMA = "apijson";         // MySQL数据库名
...
//			return "5.7.22";
	...
	public String getDBUri() {
		if (isMySQL()) {
			// 真实MySQL数据库连接
			return "jdbc:mysql://localhost:30004/apijson?useUnicode=true&characterEncoding=UTF-8&serverTimezone=GMT%2B8&useSSL=false&allowPublicKeyRetrieval=true";
		}
		if (isPostgreSQL()) {
